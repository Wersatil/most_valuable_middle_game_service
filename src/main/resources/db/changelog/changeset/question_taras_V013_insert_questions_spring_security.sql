INSERT INTO question (question_text, answer, author) VALUES
    ...
    ('Что такое AuthenticationEntryPoint?', 'Компонент Spring Security, который определяет, что делать, когда неаутентифицированный пользователь пытается получить доступ к защищённому ресурсу. Это точка входа для аутентификации, которая срабатывает при ошибке 401 Unauthorized. Проще: Если пользователь не залогинен — AuthenticationEntryPoint решает, какой ответ ему вернуть. Срабатывает, когда пользователь не аутентифицирован, но пытается куда-то перейти. Решает: вернуть ему 401 или редирект на какую-нибудь страницу (например, на страницу для ввода логина и пароля).', 'Wersatil'),
