INSERT INTO question (question_text, answer, author) VALUES
    ...
    ('Что такое Payload в JWT?', 'Содержит claims — информацию о пользователе, срок действия, роли и другие данные.', 'Wersatil'),
    ...
    ('Что делает @PreAuthorize?', 'Аннотация для проверки прав пользователя перед выполнением метода на основе выражений SpEL.', 'Wersatil'),
    ('Приведи пример использования @PreAuthorize?', '@PreAuthorize("hasRole(''ADMIN'')") — доступ только для роли ADMIN.', 'Wersatil'),
    ('Что такое CSRF?', 'Cross-Site Request Forgery — атака, когда злоумышленник заставляет пользователя выполнить нежелательное действие.', 'Wersatil'),
    ...
    ('Пример CORS настройки?', '@CrossOrigin(origins = "http://example.com") — разрешает доступ с указанного домена.', 'Wersatil'),
    ...
    ('Основные роли в OAuth2?', 'Resource Owner, Client, Authorization Server, Resource Server.', 'Wersatil'),
    ...
    ('Пример потока Authorization Code в OAuth2?', 'Клиент получает код авторизации, меняет его на Access Token через Authorization Server.', 'Wersatil'),
    ('Пример потока Client Credentials в OAuth2?', 'Клиент получает Access Token напрямую от Authorization Server, без участия пользователя.', 'Wersatil'),
    ...
    ('Что такое roles vs authorities?', 'Role — абстрактная роль пользователя, authority — конкретное право, роль может содержать несколько authorities.', 'Wersatil'),
    ('Что делает filter chain в Spring Security?', 'Обрабатывает все запросы через цепочку фильтров для аутентификации, авторизации и других проверок.', 'Wersatil'),
